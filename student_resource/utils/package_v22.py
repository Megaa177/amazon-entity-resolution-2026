"""
Packaging and Verification Script for Version 2.2 Submission
...
def main():
    ...
    match_v22 = os.path.join(base_dir, "output", "matching_results_v22.tsv")
    cand_v22 = os.path.join(base_dir, "output", "candidate_pairs_v22.tsv")

    print("=" * 60)
    print("=== Apex Entity Resolvers: Version 2.2 Packaging ===")
    print("=" * 60)

    if not os.path.exists(match_v22):
        print(f"ERROR: {match_v22} does not exist yet!")
        sys.exit(1)

    match_sz = os.path.getsize(match_v22) / (1024 * 1024)
    cand_sz = os.path.getsize(cand_v22) / (1024 * 1024)
    print(f"Output files found:")
    print(f"  matching_results_v22.tsv: {match_sz:.2f} MB")
    print(f"  candidate_pairs_v22.tsv:  {cand_sz:.2f} MB")

    # 1. Collision and Sanity Check
    print("\n[Step 1/4] Running duplicate target collision audit on matching_results_v22.tsv...")
    total_entities = 0
    empty_singletons = 0
    total_matches = 0
    target_counts = Counter()

    with open(match_v22, "r", encoding="utf-8") as f:
        header = next(f)
        for line in f:
            total_entities += 1
    ...
    shutil.copyfile(match_v22, dest_match)
    shutil.copyfile(cand_v22, dest_cand)
    print(f"  Copied v22 -> {dest_match}")
    print(f"  Copied v22 -> {dest_cand}")
    ...
    print("\n[Step 4/4] Creating Apex_Entity_Resolvers_submission_v22.zip...")
    zip_path = os.path.join(base_dir, "Apex_Entity_Resolvers_submission_v22.zip")
    if os.path.exists(zip_path):
        os.remove(zip_path)

    # Required files according to challenge specs:
    ...
    with zipfile.ZipFile(zip_path, "w", zipfile.ZIP_DEFLATED) as zf:
        ...
        if not os.path.exists(doc_path):
            doc_path = os.path.join(base_dir, "Documentation_v22.md")
        ...
        for root, dirs, files in os.walk(code_dir):
            # Skip __pycache__ and checkpoint files
