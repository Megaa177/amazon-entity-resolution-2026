...
try:
    ...
    from .features_v22 import FEATURE_NAMES_V22, token_sort_jaccard
except ImportError:
    from evaluate import compute_macro_f05
    from features_v22 import FEATURE_NAMES_V22, token_sort_jaccard


def triangulate_siblings_v22(
    s1_id: str,
    ...
    """Version 2.2 Sibling Triangulation with strict Name Gate verification."""
    if not candidates:
        return candidates
    ...
    # Promote S3 candidate only if S2 is highly confident AND they share house numbers AND name token-sort >= 0.50
    if s2_high and s3_sub:
        for s2_id, _ in s2_high:
            ...
            for s3_id, p3 in s3_sub:
                ...
                # Check Name Gate between siblings
                sibling_name_sim = token_sort_jaccard(tok_s2, tok_s3)
                if d_s2 and d_s3 and len(d_s2 & d_s3) > 0 and sibling_name_sim >= 0.50:
                    boosted[s3_id] = max(boosted[s3_id], target_tau + 0.02)
                    
    # Symmetrically promote S2 candidate
    if s3_high and s2_sub:
        for s3_id, _ in s3_high:
            ...
            for s2_id, p2 in s2_sub:
                ...
                if d_s3 and d_s2 and len(d_s3 & d_s2) > 0 and sibling_name_sim >= 0.50:
                    boosted[s2_id] = max(boosted[s2_id], target_tau + 0.02)
                    
    return list(boosted.items())


def resolve_mutual_exclusivity_v22(
    pred_dict: dict[str, set[str]], 
    s1_to_probs: dict[str, list[tuple[str, float]]],
    min_margin: float = 0.06
) -> dict[str, set[str]]:
    """Version 2.2 Global Mutual Exclusivity with Anti-Stealing Ambiguity Rejection."""
    ...
    for cid, claims in target_claims.items():
        if len(claims) == 1:
            ...
            gap = top1_p - top2_p
            # Anti-stealing gap: if claims are within min_margin, reject candidate from all
            if gap >= min_margin:
...
class MatchClassifierV22:
    """Version 2.2 Cost-Sensitive GBDT with Strict Name-Gated Filtering."""
    
    def __init__(self, n_estimators: int = 350, max_depth: int = 6, learning_rate: float = 0.07):
        # scale_pos_weight = 0.28 heavily penalizes False Positives (aligned with F0.5)
    ...
    def optimize_threshold(
        ...
        thresholds = [0.68, 0.70, 0.72, 0.75]
        single_thresholds = [0.75, 0.78, 0.80]
        deltas = [0.14, 0.16, 0.18]
        ambiguity_margins = [0.05, 0.06, 0.08]
            ...
            for s_tau in single_thresholds:
                ...
                for d in deltas:
                    ...
                    for sid, cands in s1_to_preds.items():
                        ...
                        active_cands = cands
                        if target_records:
                            active_cands = triangulate_siblings_v22(sid, cands, target_records, target_tau=tau)
                    ...
                    for amb in ambiguity_margins:
                        pred_dict = resolve_mutual_exclusivity_v22(base_preds, s1_to_preds, min_margin=amb)
        ...
        print(f"Optimal V2.2 Parameters:")
