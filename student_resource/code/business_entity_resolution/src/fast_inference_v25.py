"""
Version 2.5 Fast Universal Multi-Country Entity Resolution Inference Engine
Powered by Full Haystack Trained Model (Honest CV: 90.19% across 10.3M targets)

Pipeline:
1. Dynamic Universal Stopword Discovery per Country
2. Universal Candidate Blocking (Order-invariant bigrams, tokens, address keys, plot codes)
3. Full 25-Dense Feature Extraction without arbitrary heuristic gating
4. Cost-Sensitive GBDT Scoring (trained on 259,498 real distractors from 10.3M haystack)
5. Sibling Triangulation & Distinctiveness-Adaptive Metric Thresholding
6. Anti-Stealing Global Mutual Exclusivity (Zero duplicate target collisions guaranteed)
"""
...
try:
    ...
    from .features_v23 import compute_pair_features_v23
    from .model_v23 import resolve_mutual_exclusivity_v23, triangulate_siblings_v23
except ImportError:
    from preprocessing_v23 import (
        clean_business_name_v23,
        clean_address_v23,
...
def process_country_v25(country: str, part_dir: str, clf_data: dict, out_match_path: str, out_cand_path: str):
    print(f"\n" + "=" * 65)
    print(f"=== Starting Version 2.5 Processing for: {country} ===")
    print(f"=" * 65)
    t_start = time.time()
    ...
    print(f"Loaded Full Haystack Calibrated Decision Parameters:")
    ...
    print(f"Extracting 25 dense features & scoring pairs with model_full_haystack...")
    t_score = time.time()
    ...
    # 5. Sibling Triangulation & Distinctiveness-Adaptive Thresholding
    print("Applying Sibling Triangulation and Distinctiveness-Adaptive Thresholding...")
    initial_matches = {}
    ...
    # Explicit garbage collection to release memory before next country
...
def main():
    ...
    model_path = os.path.join(base_dir, 'code', 'business_entity_resolution', 'src', 'model_full_haystack.joblib')
    
    out_match = os.path.join(base_dir, 'output', 'matching_results_v25.tsv')
    out_cand = os.path.join(base_dir, 'output', 'candidate_pairs_v25.tsv')
    os.makedirs(os.path.dirname(out_match), exist_ok=True)
    os.makedirs(os.path.dirname(out_cand), exist_ok=True)
    
    # Initialize output files with official competition headers
    ...
    print(f"Loading Full Haystack Trained Model Checkpoint: {model_path}...")
    ...
    for country in ['France', 'US', 'India']:
        process_country_v25(country, part_dir, clf_data, out_match, out_cand)
        
    total_time = time.time() - t0
    print("\n" + "=" * 70)
    print(f"VERSION 2.5 FULL HAYSTACK INFERENCE COMPLETE IN {total_time:.1f}s ({total_time/60:.1f} minutes)!")
