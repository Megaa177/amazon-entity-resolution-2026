...
def token_sort_jaccard(tokens1: list[str], tokens2: list[str]) -> float:
    """Alphabetically sorted token Jaccard (completely order-invariant)."""
...
def compute_pair_features_v22(
    s1_rec: dict, 
    cand_rec: dict, 
    rank: int = 0
) -> list[float]:
    """Compute dense 22-feature vector with strict Name Gate and Anti-Co-Location signals."""
    ...
    # 2. Token-Sort Jaccard (Order-invariant: 'Team Ecole' == 'Ecole Team' -> 1.0)
    ...
    # 6. Token Overlap Ratio (Recall coverage of S1 in candidate)
    ...
    # 12. Pure Street Words Jaccard (excluding numbers and generic street words)
    ...
    # Identical building number/address BUT low name similarity (< 0.45)
    # This explicitly detects different businesses at the same address (e.g. Team Ecole vs Team Services)
    co_location_conflict = 1.0 if (num_match == 1.0 and name_sort_jaccard < 0.45 and name_exact == 0.0) else 0.0

    # 14. First Token Match (Corporate anchor)
